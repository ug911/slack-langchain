...
class AsyncStreamingSlackCallbackHandler(AsyncCallbackHandler):
    ...
    This class manages the interaction between a language model's streaming output and Slack,
    allowing for real-time updates to be sent to a specified Slack channel.
    """
    ...
    async def on_llm_new_token(self, token: str, **kwargs: Any) -> None:
        """
        Called when a new token is generated by the LLM. Updates the current message and triggers a Slack update.Run on new LLM token. Only available when streaming is enabled."""
    ...
    async def handle_llm_error(self, e: Exception) -> None:
        """
        Handles errors from the LLM by posting an error message to Slack.Post error message to channel with provided channel_id and thread_ts."""
        try:
    ...
    async def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        """
        Called when the LLM finishes processing. Finalizes the message update in Slack.Run when LLM ends running."""
        try:
